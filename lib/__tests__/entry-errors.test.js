...
describe('scripts array entry compiles when all entry points exist', () => {
  ...
  it('entry points in different dirs nest output under the common ancestor', async() => {
    fs.mkdirSync(path.join(TMP, 'src', 'a'), { recursive: true })
    fs.mkdirSync(path.join(TMP, 'src', 'b'), { recursive: true })
    fs.writeFileSync(path.join(TMP, 'src', 'a', 'main.js'), 'console.log(1)\n')
    fs.writeFileSync(path.join(TMP, 'src', 'b', 'main.js'), 'console.log(2)\n')

    // previously the output path was computed as dist/js/main.js (basename only),
    // so fileSize threw ENOENT and aborted the compile
    await new Scripts({
      scripts: { in: [path.join(TMP, 'src', 'a', 'main.js'), path.join(TMP, 'src', 'b', 'main.js')], out: path.join(TMP, 'dist', 'js') },
      includePaths: []
    }).compile()

    expect(fs.existsSync(path.join(TMP, 'dist', 'js', 'a', 'main.js'))).toBe(true)
    expect(fs.existsSync(path.join(TMP, 'dist', 'js', 'b', 'main.js'))).toBe(true)
    expect(fs.existsSync(path.join(TMP, 'dist', 'js', 'main.js'))).toBe(false)
  })
})
