...
//
// The walk itself belongs to `unknown-keys` and is specified by its tests, not
// these: what is asserted below is that the schema Poops ships closes the blocks
// Poops owns and leaves open the ones it does not, and that the CLI reports what
// comes back.
//
...
const unknownIn = (config) => unknownConfigKeys(config, schema).map(({ path, key }) => `${path}.${key}`)
...
describe('unknown top-level config keys', () => {
  ...
  it('says nothing about a key a direct dependency vouches for, though nothing by that name is depended on', () => {
    write('package.json', { name: 'fixture', version: '1.0.0', devDependencies: { laxative: '^1.0.0' } })
    fs.mkdirSync(path.join(TMP, 'node_modules', 'laxative'), { recursive: true })
    write('node_modules/laxative/package.json', { name: 'laxative', version: '1.0.0', poops: { companionKeys: ['septic'] } })
    write('poops.json', { styles: STYLES, septic: { db: 'data/app.db' } })
    expect(runPoops()).not.toContain('septic')
...
describe('projectPackageNames', () => {
  ...
  it('adds the keys a direct dependency vouches for, one directory deep and no further', () => {
    write('package.json', { name: 'fixture', dependencies: { a: '1' } })
    fs.mkdirSync(path.join(TMP, 'node_modules', 'a'), { recursive: true })
    write('node_modules/a/package.json', { name: 'a', poops: { companionKeys: ['vouched', 42] } })
    expect([...projectPackageNames(TMP)].sort()).toEqual(['a', 'vouched'])
    fs.rmSync(path.join(TMP, 'node_modules'), { recursive: true, force: true })
  })

  it('treats a dependency with no manifest on disk as its name alone', () => {
    write('package.json', { name: 'fixture', dependencies: { ghost: '1' } })
    expect(projectPackageNames(TMP)).toEqual(new Set(['ghost']))
  })
