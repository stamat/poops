...
import { parseFrontMatter, applyHeadingSlugs } from './helpers.js'
...
export function generateSitemap(pageEntries, outputDir, siteUrl, config) {
  ...
  for (const entry of pageEntries) {
    ...
    // lastmod is last *modified*: an edit date outranks the publication date.
    // An unparseable date is dropped rather than thrown on — toISOString on an
    // Invalid Date takes the whole build down over one typo in front matter.
    // guarded rather than fed straight to Date: `new Date(null)` is the epoch,
    // not an Invalid Date, so a null date would advertise 1970 to crawlers
    const stamp = entry.updated || entry.date
    const lastmod = stamp ? new Date(stamp) : null
    if (lastmod && !isNaN(lastmod)) {
      xml += `    <lastmod>${lastmod.toISOString().slice(0, 10)}</lastmod>\n`
    }
    xml += '  </url>\n'
  }
...
function feedContentHtml(e) {
  ...
  // Nothing renders this through the template engine, so a heading built from a
  // template tag has no text to slug: applyHeadingSlugs emits it plain here
  // rather than letting the pending marker reach a feed reader.
  return applyHeadingSlugs(renderMarkdownCached(e._src, pageSource(e))).trim()
}
