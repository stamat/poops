...
export const markdownRenderer = {
  ...
  // still focusable is a link in the tab order that no screen reader can name —
  // a stop on nothing, and a WCAG 4.1.2 failure. It also carried an `aria-label`,
  ...
  heading({ tokens, depth, text: raw }) {
    ...
    return `<h${depth} id="${id}">${text}<a class="heading-anchor" href="#${id}" tabindex="-1" aria-hidden="true"></a></h${depth}>\n`
  }
}
