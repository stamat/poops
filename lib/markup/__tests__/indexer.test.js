...
import { extractKeywords, stripMachinery, generateSearchIndex, generateSitemap, generateLlmsTxt, generateLlmsFull, generateRobotsTxt, generateIndexFiles, buildNavTree, generateNav, generateFeeds, _getKeywordCache } from '../indexer.js'
...
describe('generateSitemap', () => {
  ...
  it('drops the lastmod of an unparseable date instead of failing the build', () => {
    expect(() => generateSitemap(
      [{ url: 'page.html', title: 'Page', date: 'last tuesday' }],
      relativeTmpDir(), 'https://example.com', 'sitemap.xml'
    )).not.toThrow()
    const xml = fs.readFileSync(path.join(tmpDir, 'sitemap.xml'), 'utf-8')
    expect(xml).toContain('<loc>https://example.com/page.html</loc>')
    expect(xml).not.toContain('<lastmod>')
  })

  it('takes an edit date over a publication date, since lastmod means last modified', () => {
    generateSitemap(
      [{ url: 'page.html', title: 'Page', date: '2024-01-15', updated: '2026-05-20' }],
      relativeTmpDir(), 'https://example.com', 'sitemap.xml'
    )
    const xml = fs.readFileSync(path.join(tmpDir, 'sitemap.xml'), 'utf-8')
...
describe('generateLlmsFull', () => {
  ...
  it('strips the machinery a markdown source still carries', () => {
    const body = [
    ...
    const txt = fs.readFileSync(path.join(tmpDir, 'llms-full.txt'), 'utf-8')
    expect(txt).toContain('The prose the capture wrapped.') // what the tags wrapped survives
    expect(txt).not.toContain('{% set body %}')
    expect(txt).not.toContain('{{ body }}')
    expect(txt).not.toContain('a note to the author')
    expect(txt).not.toContain('.toc { padding: 1rem; }')
    expect(txt).not.toContain('page.css')
    expect(txt).not.toContain('page.js')
    expect(txt).not.toMatch(/\n{3,}/) // and no hole where a removed line was
  })
})

describe('stripMachinery', () => {
  it('leaves a source with neither template tags nor assets untouched', () => {
    const md = '# Title\n\nProse with a { brace } in it.\n'
    expect(stripMachinery(md)).toBe(md)
  })

  it('keeps machinery inside fenced blocks and inline spans', () => {
    const md = [
      'Write `{{ page.title }}` in a template, and `<script src="x.js">` to load it.',
      '',
      '```njk',
      '{% for item in items %}',
    ...
    const out = stripMachinery(md)
    expect(out).toContain('`{{ page.title }}`')
    expect(out).toContain('`<script src="x.js">`')
    expect(out).toContain('{% for item in items %}')
    expect(out).toContain('<script>console.log("{{ item }}")</script>')
    expect(out).not.toContain('{% set x %}')
    expect(out).toContain('kept') // the capture's body, not its tags
  })

  it('drops inline styles and scripts but keeps the prose html around them', () => {
    const out = stripMachinery('<p class="intro">Read me.</p>\n\n<style>p { color: red }</style>\n\n<script>\nalert(1)\n</script>\n')
    expect(out).toContain('<p class="intro">Read me.</p>')
    expect(out).not.toContain('color: red')
    expect(out).not.toContain('alert(1)')
  })

  it('unwraps a {% raw %} block and keeps what it protected', () => {
    const out = stripMachinery('{% raw %}Literally {{ this }}.{% endraw %}\n\n{% if x %}Text.{% endif %}\n')
    expect(out).toContain('Literally {{ this }}.')
    expect(out).not.toContain('{% raw %}')
    expect(out).toContain('Text.') // a branch contributes its text either way
    expect(out).not.toContain('{% if x %}')
  })

  it('survives an unclosed fence at the end of a source', () => {
    const out = stripMachinery('{% set x %}\n\n```js\nconst a = "{{ b }}"\n')
    expect(out).toContain('const a = "{{ b }}"')
    expect(out).not.toContain('{% set x %}')
  })

  it('takes a code span inside a template comment with the comment', () => {
    const out = stripMachinery('{# a note about `prose` layouts #}\nProse.\n')
    expect(out).not.toContain('prose')
    expect(out.trim()).toBe('Prose.') // the emptied line is left for the caller to trim
  })

  it('leaves the whitespace around a kept construct exactly as it was', () => {
    const md = 'The theme\'s `prose` layout, two  spaces  in.\n'
    expect(stripMachinery(md)).toBe(md)
    expect(stripMachinery(`{# note #}\n${md}`)).toBe(`\n${md}`)
  })
})
...
describe('buildNavTree', () => {
  ...
  it('files a navGroup page under a virtual section beside its ungrouped siblings', () => {
    const tree = buildNavTree([
      { url: 'guide/intro', title: 'Intro', order: 1, isIndex: false },
      { url: 'guide/copy', title: 'Copy', order: 2, navGroup: 'No pattern', isIndex: false }
    ])
    const guide = tree[0]
    expect(guide.children.map(n => n.title)).toEqual(['Intro', 'No pattern'])
    const group = guide.children[1]
    expect(group.url).toBeUndefined()
    expect(group.children.map(n => n.title)).toEqual(['Copy'])
    // the page keeps the url its file path gives it — grouping moves nothing
    expect(group.children[0].url).toBe('guide/copy')
    expect(group.children[0].order).toBe(2)
  })

  it('gives the group the title as written, not a humanized segment', () => {
    const tree = buildNavTree([
      { url: 'docs/copy', title: 'Copy', navGroup: 'no APG pattern', isIndex: false }
    ])
    expect(tree[0].children.map(n => n.title)).toEqual(['no APG pattern'])
  })

  it('opens one group per directory, so the same name in two places does not merge', () => {
    const tree = buildNavTree([
      { url: 'guide/copy', title: 'Copy', navGroup: 'Extras', isIndex: false },
      { url: 'api/probe', title: 'Probe', navGroup: 'Extras', isIndex: false }
    ])
    expect(tree.map(n => n.title)).toEqual(['Api', 'Guide'])
    expect(tree[0].children[0].children.map(n => n.title)).toEqual(['Probe'])
    expect(tree[1].children[0].children.map(n => n.title)).toEqual(['Copy'])
  })

  it('sorts a group where its first child would have sorted', () => {
    const tree = buildNavTree([
      { url: 'guide/last', title: 'Last', order: 9, isIndex: false },
      { url: 'guide/grouped', title: 'Grouped', order: 1, navGroup: 'Extras', isIndex: false }
    ])
    expect(tree[0].children.map(n => n.title)).toEqual(['Extras', 'Last'])
    expect(tree[0].children[0].order).toBe(1)
  })

  it('keeps a directory named like a group out of that group', () => {
    const tree = buildNavTree([
      { url: 'guide/extras', title: 'Extras index', isIndex: false },
      { url: 'guide/extras/notes', title: 'Notes', isIndex: false },
      { url: 'guide/copy', title: 'Copy', navGroup: 'extras', isIndex: false }
    ])
    const children = tree[0].children
    expect(children.length).toBe(2)
    const directory = children.find(n => n.url === 'guide/extras')
    const group = children.find(n => n.url === undefined)
    expect(directory.children.map(n => n.title)).toEqual(['Notes'])
    expect(group.children.map(n => n.title)).toEqual(['Copy'])
  })

  it('refuses navGroup on a section index page rather than splitting the section', () => {
    const tree = buildNavTree([
      { url: 'guide', title: 'Guide', navGroup: 'Extras', isIndex: false },
      { url: 'guide/child', title: 'Child', isIndex: false }
    ])
    expect(tree.map(n => n.title)).toEqual(['Guide'])
    expect(tree[0].url).toBe('guide')
    expect(tree[0].children.map(n => n.title)).toEqual(['Child'])
  })

  it('refuses navGroup on the home page, which has no siblings to group among', () => {
    const tree = buildNavTree([
      { url: '', title: 'Home', navGroup: 'Extras', isIndex: false },
      { url: 'about', title: 'About', isIndex: false }
    ])
    expect(tree.map(n => n.title)).toEqual(['About', 'Home'])
    expect(tree.find(n => n.title === 'Home').url).toBe('')
  })

  it('ignores a navGroup that is not a usable string', () => {
    const tree = buildNavTree([
      { url: 'guide/copy', title: 'Copy', navGroup: '   ', isIndex: false },
      { url: 'guide/probe', title: 'Probe', navGroup: true, isIndex: false }
    ])
    expect(tree[0].children.map(n => n.title)).toEqual(['Copy', 'Probe'])
  })
