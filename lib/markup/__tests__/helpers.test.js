...
describe('excerpt', () => {
  it('returns the first prose paragraph as plain text', () => {
    expect(excerpt('First **para** here.\n\nSecond para.')).toBe('First para here.')
  })

  it('skips leading headings and HTML comments', () => {
    expect(excerpt('# Title\n\n<!-- note -->\n\nReal intro text.')).toBe('Real intro text.')
  })

  it('truncates to max on a word boundary with an ellipsis', () => {
    const out = excerpt('word '.repeat(60), 20)
    expect(out.length).toBeLessThanOrEqual(21)
    expect(out.endsWith('…')).toBe(true)
    expect(out).not.toContain('  ')
  })

  it('returns empty string for empty or heading-only content', () => {
    expect(excerpt('')).toBe('')
