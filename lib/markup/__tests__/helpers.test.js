...
describe('breadcrumbCrumbs', () => {
  it('derives a home > ancestors > page trail from URL depth', () => {
    expect(breadcrumbCrumbs(
      { title: 'Pages', url: 'docs/static-site/pages.html' },
      { url: 'https://ex.com/' }
    )).toEqual([
      { name: 'Home', url: 'https://ex.com' },
      { name: 'Docs', url: 'https://ex.com/docs' },
      { name: 'Static Site', url: 'https://ex.com/docs/static-site' },
      { name: 'Pages', url: 'https://ex.com/docs/static-site/pages.html' }
    ])
  })
  ...
  it('drops the home crumb when home is off, suppressing single-crumb pages', () => {
    ...
    expect(breadcrumbCrumbs(
      ...
      { name: 'Blog', url: 'https://ex.com/blog' },
      { name: 'Post', url: 'https://ex.com/blog/post.html' }
    ])
  })
  ...
  it('lets front matter override the site-level home label', () => {
    ...
    expect(crumbs[0]).toEqual({ name: 'Start', url: 'https://ex.com' })
  })

  it('uses root-relative URLs without site.url', () => {
    expect(breadcrumbCrumbs({ title: 'Post', url: 'blog/post.html' }, {})).toEqual([
      { name: 'Home', url: '/' },
      { name: 'Blog', url: '/blog' },
      { name: 'Post', url: '/blog/post.html' }
    ])
  })
})

describe('buildBreadcrumb', () => {
  it('renders a nav/ol trail, last crumb as aria-current text', () => {
    const html = buildBreadcrumb({ title: 'Post', url: 'blog/post.html' }, { url: 'https://ex.com' })
    expect(html).toBe(
      '<nav class="breadcrumb" aria-label="Breadcrumb"><ol>' +
      '<li><a href="https://ex.com">Home</a></li>' +
      '<li><a href="https://ex.com/blog">Blog</a></li>' +
  ...
  it('escapes crumb names and URLs', () => {
