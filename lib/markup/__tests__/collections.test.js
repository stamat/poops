...
describe('collection item paths', () => {
  // filePath feeds "edit this page on GitHub" links, so it must be posix on
  // every platform — the same guarantee page.filePath gives a non-collection page
  it('reports filePath with posix separators', () => {
    fs.writeFileSync(path.join(TMP, 'src', 'posts', 'hello.md'), '---\ntitle: Hello\ndate: 2024-01-15\n---\nhi\n')

    const [item] = getSingleCollectionData('src', 'posts')

    expect(item.filePath).toBe('src/posts/hello.md')
    expect(item.filePath).not.toContain('\\')
  })
})
