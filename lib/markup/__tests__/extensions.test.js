...
import { ImageExtension, GoogleFontsExtension, HighlightExtension, PaginationExtension } from '../engines/nunjucks.js'
...
describe('liquid image tag', () => {
  ...
  it('keeps a quoted value that contains commas intact', async() => {
    const engine = new LiquidEngine('.', [])
    engine.registerTags(() => '/nonexistent')
    // A naive split(',') would break this into two garbled attributes.
    const html = await engine.renderString('{% image "photo.svg", data-sizes: "(max-width: 600px) 100vw, 50vw" %}', {})
    expect(html).toContain('data-sizes="(max-width: 600px) 100vw, 50vw"')
  })
})

describe('PaginationExtension', () => {
  it('has the "pagination" tag', () => {
    const ext = new PaginationExtension()
    expect(ext.tags).toEqual(['pagination'])
  })

  it('run renders links and page numbers', () => {
    const ext = new PaginationExtension()
    const context = { lookup: (key) => key === 'relativePathPrefix' ? '../' : '' }
    const result = ext.run(context, {
      totalPages: 3,
      pageNumber: 2,
      nextPageUrl: 'blog/3',
      prevPageUrl: 'blog'
...
describe('liquid pagination tag', () => {
  it('renders links and page numbers', async() => {
    const engine = new LiquidEngine('.', [])
    engine.registerTags(() => '/nonexistent')
    const html = await engine.renderString('{% pagination pager %}', {
      relativePathPrefix: '../',
      pager: { totalPages: 3, pageNumber: 2, nextPageUrl: 'blog/3', prevPageUrl: 'blog' }
    })
    expect(html).toContain('href="../blog/3"')
    expect(html).toContain('2 of 3')
    expect(html).toContain('href="../blog"')
