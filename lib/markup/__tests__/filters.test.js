...
import { slugify, jsonify, markdown, concat, push, svg, dateFilter, srcsetFilter } from '../filters.js'
...
describe('svg', () => {
  it('returns SVG content from file', () => {
    const result = svg(path.join(FIXTURES, 'icon.svg'))
    expect(result).toContain('<svg')
    expect(result).toContain('<circle')
  })

  it('returns empty string for nonexistent file', () => {
    expect(svg(path.join(FIXTURES, 'nope.svg'))).toBe('')
  })

  it('returns empty string for non-SVG file', () => {
    expect(svg(path.join(FIXTURES, 'not-svg.txt'))).toBe('')
  })
