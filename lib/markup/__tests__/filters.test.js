...
beforeAll(() => {
  ...
  engine.registerFilters({ dateFormat: 'YYYY-MM-DD', markupOut: 'nonexistent' })
})
...
describe('groupby filter', () => {
  ...
  it('splits array-valued keys so a post lands under each tag', async() => {
    const items = [{ tags: ['js', 'css'] }, { tags: ['js'] }]
    const out = await render(
      '{% for g in items | groupby("tags") %}{{ g.key }}:{{ g.items.length }} {% endfor %}',
      { items }
    )
    expect(out.trim()).toBe('js:2 css:1')
  })
})
