...
describe('path handling', () => {
  // startsWith(root) also accepts a sibling named like root — `dist-old` for
  // `dist` — so a `../` in a template's argument escaped the output dir
  it('does not read a cache from a sibling directory whose name starts with the output dir', () => {
    const dist = path.join(TMP, 'dist')
    const sibling = path.join(TMP, 'dist-old')
    fs.mkdirSync(dist, { recursive: true })
    writeCache(sibling, { 'secret.jpg': { width: 10, height: 10, exif: null, outputs: [] } })

    expect(getImageEntry('../dist-old/secret.jpg', dist)).toBe(null)
    expect(listImages('../dist-old', dist)).toEqual([])
  })

  // a cache written on Windows carries backslash keys; every lookup here is posix
  it('matches cache keys written with backslash separators', () => {
    const dist = path.join(TMP, 'dist')
    writeCache(dist, { 'photos\\a.jpg': { width: 100, height: 50, exif: null, outputs: [] } })

    const found = getImageEntry('photos/a.jpg', dist)
    expect(found && found.entry.width).toBe(100)
    expect(listImages('photos', dist).map((i) => i.path)).toEqual(['photos/a.jpg'])
  })
})
