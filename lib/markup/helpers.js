...
// wordCount) are added only for the article case. Anything in `page.jsonld`
// (an object) is merged over the generated defaults, so a page can add or
// replace any field — including @type — as a full escape hatch. Values come
// from front matter, so the JSON is escaped for a <script> context to prevent
// a `</script>` break-out (XSS).
export function buildJsonLd(page, site = {}) {
  ...
  const publisher = site.title || site.name
  ...
  // Drop empties so the block stays lean (GEO parsers ignore them anyway).
  for (const k of Object.keys(data)) {
    if (data[k] === undefined || data[k] === null || data[k] === '') delete data[k]
  }

  // Neutralize `<`, `>`, `&` so front-matter values can't break out of the
  // <script> or inject entities. The tag itself is intended raw markup.
...
export function buildOpenGraph(page, site = {}) {
  ...
  const tags = {
    ...
    'og:description': page.description || site.description,
