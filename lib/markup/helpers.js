...
// Builds a breadcrumb trail — a { name, url } array from the site root down to
...
// `url` is absolute when `site.url` is set (JSON-LD requires it, and it works as
// an href too), else root-relative.
...
export function breadcrumbCrumbs(page, site = {}) {
  ...
  const abs = (u) => {
    ...
    const rel = s.replace(/^\/+/, '')
    if (baseUrl) return rel ? `${baseUrl}/${rel}` : baseUrl
    return rel ? `/${rel}` : '/'
  }

  const parts = rawUrl.split('/').filter(Boolean)
  const crumbs = []
  if (cfg.home) crumbs.push({ name: cfg.homeLabel, url: abs('') })

  // Every part but the last is an ancestor directory; accumulate its path.
  let acc = ''
  for (let i = 0; i < parts.length - 1; i++) {
    acc = acc ? `${acc}/${parts[i]}` : parts[i]
    crumbs.push({ name: humanizeSegment(parts[i]), url: abs(acc) })
  }
  crumbs.push({ name: page.title || humanizeSegment(parts[parts.length - 1]), url: abs(rawUrl) })
...
// posts, nested pages). Same crumbs as the JSON-LD, but the last crumb renders
// as plain text with aria-current (the current page is not a link). Returns ''
// when there's no trail. Names/URLs come from front matter, so they're escaped.
export function buildBreadcrumb(page, site = {}) {
  const crumbs = breadcrumbCrumbs(page, site)
  if (crumbs.length < 2) return ''
  const items = crumbs.map((c, i) => {
    const name = escapeAttr(c.name)
    if (i === crumbs.length - 1) return `<li><span aria-current="page">${name}</span></li>`
    return `<li><a href="${escapeAttr(c.url)}">${name}</a></li>`
  })
