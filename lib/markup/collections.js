...
export function getSingleCollectionData(markupInDir, collectionName) {
  ...
  globSync(toPosix(path.resolve(process.cwd(), markupInDir, collectionName, '**/*.+(html|njk|liquid|md)')), { ignore: ['**/index.+(html|njk|liquid|md)'] }).forEach((file) => {
    ...
    // Item excerpts feed listing pages and feed <description>s, and there is no
    // page context to render an item with here — a first paragraph made of
    // template tags leaves the excerpt empty rather than guessing at it. The item
    // reaches its own page through compileEntry, which does resolve it.
    frontMatter.excerpt = excerpt(path.extname(file) === '.md' ? renderMarkdownCached(file, content) : content)
    frontMatter.fileName = path.basename(file)
    // posix, like the page.filePath a template gets for a non-collection page —
