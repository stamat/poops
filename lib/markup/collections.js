...
function renderPages({ pages, urlBase, collection, extra, pageProps, collectionName, collectionData, file, markupOutDir, compileEntryFn, baseURL }) {
  const promises = []
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    // pages use it to feed the breadcrumb a correct term trail
    if (pageProps) context._page = pageProps
...
export function generateTaxonomyPages(collectionData, markupInDir, markupOutDir, compileEntryFn, baseURL) {
  if (!collectionData) return []
    ...
    for (const tax of collection.taxonomies) {
      ...
      for (const term of tax.terms) {
        compilePromises.push(...renderPages({
          ...
          pageProps: { _taxonomy: { collection: collectionName, term: term.term, termUrl: term.url } },
          collectionName,
          collectionData,
          file,
          markupOutDir,
