...
function renderPages({ pages, urlBase, collection, extra, pageProps, titleBase, titleAlways, collectionName, collectionData, file, markupOutDir, compileEntryFn, baseURL }) {
  const promises = []
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    if (titleBase && (titleAlways || pageNumber > 1)) {
      props.title = pageNumber === 1 ? titleBase : `${titleBase} — Page ${pageNumber}`
    }
