...
// plus every key those packages vouch for. A top-level config key naming a
// dependency is a companion reading the same poops.json — septic's `septic`
// block, say — not a typo, so poops.js stays quiet about it. A key can also
// arrive one step removed: laxative brings septic, so the app's package.json
// says `laxative` and never `septic`. For that, a direct dependency vouches
// through its own manifest — `"poops": { "companionKeys": ["septic"] }` — and
// only direct dependencies are read, so the check stays one directory deep.
// Poops never loads any of these packages; it only stops calling the key a
// mistake, which is why declaration is enough and an install is not checked.
// An unreadable or absent package.json degrades to the empty set — or, for a
// dependency's own manifest, to its name alone: every unknown key is warned
// about again, which is exactly the behaviour before this existed.
export function projectPackageNames(projectPath) {
  const root = projectPath || process.cwd()
  const packagesFilePath = path.join(root, 'package.json')
  ...
  for (const name of names) {
    let dep
    try {
      dep = JSON.parse(fs.readFileSync(path.join(root, 'node_modules', name, 'package.json'), 'utf-8'))
    } catch {
      continue
    }
    const vouched = dep.poops?.companionKeys
    if (!Array.isArray(vouched)) continue
    for (const key of vouched) if (typeof key === 'string') keys.add(key)
  }
  return keys
}
...
// Fills `{{ version }}` and its siblings from the project's package.json, in a
// banner and anywhere else config asks for them. poops.json is JSON: a value
// wanting the version has no way to compute one, so without this the number
// lives in two files that drift apart with nothing failing when they do.
//
// Strings are replaced, objects and arrays walked, everything else returned as
// it came — a copy, never the caller's config object, since site data is handed
// to the template engine and a mutated config would surprise a second reader.
//
// An unknown token — a typo, or a package.json missing the field — is left as
// written. "undefined" in a banner or a footer reads as Poops being broken,
// where `{{ verison }}` sitting there is the mistake, named.
export function fillPackageTokens(value, packagesPath) {
  const packagesFilePath = path.join(packagesPath || process.cwd(), 'package.json')
  if (!pathExists(packagesFilePath)) return value
  const { name, version, homepage, description, license, author } = JSON.parse(fs.readFileSync(packagesFilePath, 'utf-8'))
  const tokens = { name, version, homepage, description, license, author, year: new Date().getFullYear() }

  const fill = (v) => {
    if (typeof v === 'string') return v.replace(/{{\s*(\w+)\s*}}/g, (token, key) => tokens[key] ?? token)
    if (Array.isArray(v)) return v.map(fill)
    if (v && typeof v === 'object') return Object.fromEntries(Object.entries(v).map(([key, item]) => [key, fill(item)]))
    return v
  }
