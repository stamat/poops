...
// Follows $ref, bounded so a schema that points at itself cannot hang a build
// this only ever advises on.
function resolveRef(node, schema, depth = 0) {
  if (!node || !node.$ref || depth > 10) return node
  return resolveRef(schema.definitions[node.$ref.replace('#/definitions/', '')], schema, depth + 1)
}

// The branch of a oneOf/allOf describing the shape this value actually has —
// `styles` accepts one entry or an array of them, and only one of those can be
// checked against what is written. Two branches fitting equally means nothing
// can be said without validating types, so the walk stops rather than guess a
// key set and warn against the wrong one.
function shapeFor(node, value, schema) {
  node = resolveRef(node, schema)
  if (!node) return null
  const branches = node.oneOf || node.anyOf || node.allOf
  if (!branches) return node
  const fits = (branch) => branch && (Array.isArray(value) ? branch.type === 'array' : branch.type === 'object' || !!branch.properties)
  const matches = branches.map((branch) => shapeFor(branch, value, schema)).filter(fits)
  return matches.length === 1 ? matches[0] : null
}

function walkConfig(value, node, schema, at, found) {
  if (!value || typeof value !== 'object') return
  node = shapeFor(node, value, schema)
  if (!node) return
  if (Array.isArray(value)) {
    value.forEach((item, index) => walkConfig(item, node.items, schema, `${at}[${index}]`, found))
    return
  }
  const known = node.properties || {}
  for (const [key, child] of Object.entries(value)) {
    if (key in known) walkConfig(child, known[key], schema, at ? `${at}.${key}` : key, found)
    else if (node.additionalProperties === false) found.push({ at, key, valid: Object.keys(known) })
  }
}

// Misspelt keys below the top level, which poops.js warns about. The key sets
// come off the published schema rather than a second list in here: a block
// written `additionalProperties: false` is one Poops owns and can rule on, and
// `true` — `images`, `site` — is one somebody else names the keys of. The root
// is neither, so nothing here fires at the top level; poops.js owns that check,
// which alone can tell a companion's block from a typo.
//
