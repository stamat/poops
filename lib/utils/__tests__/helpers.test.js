...
describe('buildTime', () => {
  ...
  it('formats exact seconds', () => {
    expect(buildTime(0, 2000)).toBe('2s')
  })

  it('formats minutes with remainder', () => {
    expect(buildTime(0, 125000)).toBe('2m 5s')
  })
...
describe('fileSize', () => {
  ...
  it('formats megabyte files with no remainder', () => {
    ...
    expect(fileSize(filePath)).toBe('2MB')
  })

  it('formats gigabyte files with MB remainder', () => {
    const filePath = path.join(TMP, 'huge.txt')
    fs.mkdirSync(TMP, { recursive: true })
    // sparse file — sets stat.size without writing 1.5GB to disk
    fs.writeFileSync(filePath, '')
    fs.truncateSync(filePath, 1_500_000_000)
    expect(fileSize(filePath)).toBe('1GB 500MB')
  })
})

// --- Package tokens ---
//
// The same tokens a banner carries, filled wherever config asks for them. Not
// covered here: nunjucks/liquid expressions in config values — the tokens are a
// flat substitution, and anything needing logic belongs in a template.

describe('fillPackageTokens', () => {
  it('replaces all template variables from package.json', () => {
    const template = '/* {{name}} v{{version}} | {{license}} | {{author}} | {{homepage}} | {{description}} */'
    const result = fillPackageTokens(template, FIXTURES)
    expect(result).toBe('/* test-pkg v1.2.3 | MIT | Test Author | https://example.com | A test package */')
  })

  it('replaces {{year}} with current year', () => {
    const result = fillPackageTokens('(c) {{ year }}', FIXTURES)
    expect(result).toBe(`(c) ${new Date().getFullYear()}`)
  })

  it('returns template unchanged when package.json is missing', () => {
    const template = '/* {{name}} */'
    expect(fillPackageTokens(template, '/nonexistent/path')).toBe(template)
  })

  it('fills every string in an object, however deeply nested', () => {
    const site = {
      footer: 'v{{ version }}',
      links: [{ title: '{{ name }} on npm' }],
      count: 3,
      enabled: true
    }
    expect(fillPackageTokens(site, FIXTURES)).toEqual({
      footer: 'v1.2.3',
      links: [{ title: 'test-pkg on npm' }],
      count: 3,
      enabled: true
    })
  })
  ...
  it('leaves the config it was given untouched', () => {
    const site = { footer: 'v{{ version }}' }
    fillPackageTokens(site, FIXTURES)
    expect(site.footer).toBe('v{{ version }}')
  })

  it('leaves a token it does not know as written, never the word undefined', () => {
    expect(fillPackageTokens('{{ verison }}', FIXTURES)).toBe('{{ verison }}')
  })
})
...
describe('stripDirNavSegments', () => {
  it('removes leading ../ segments', () => {
    expect(stripDirNavSegments('../src/file.js')).toBe('src/file.js')
  })

  it('removes multiple leading ../ segments', () => {
    expect(stripDirNavSegments('../../src/file.js')).toBe('src/file.js')
  })
  ...
  it('resolves mid-path ../ via normalize', () => {
    expect(stripDirNavSegments('src/../lib/file.js')).toBe('lib/file.js')
  })

  it('strips leading ../ from glob patterns', () => {
    expect(stripDirNavSegments('../**/*.js')).toBe('**/*.js')
  })
...
describe('pathContainsPathSegment', () => {
  ...
  it('matches on segment boundaries, not substrings', () => {
    // `src` must not match inside a longer segment name
    expect(pathContainsPathSegment('mysrc/app.js', 'src')).toBe(false)
    expect(pathContainsPathSegment('dist/src-maps/x.css', 'src')).toBe(false)
    // but a real `src` segment anywhere matches
    expect(pathContainsPathSegment('src/app.js', 'src')).toBe(true)
    expect(pathContainsPathSegment('a/src/b.js', 'src')).toBe(true)
  })
})

describe('replacePathSegment', () => {
  it('swaps the in-dir prefix for the out-dir', () => {
    expect(replacePathSegment('src/assets/x.png', 'src/assets', 'dist/assets')).toBe('dist/assets/x.png')
  })

  it('rewrites the real segment, not a substring inside an earlier one', () => {
    expect(replacePathSegment('mysrc/src/x.js', 'src', 'dist')).toBe('mysrc/dist/x.js')
  })

  it('returns the posix path unchanged when the segment is absent', () => {
    expect(replacePathSegment('a/b/c.js', 'src', 'dist')).toBe('a/b/c.js')
  })
})
...
describe('deriveWatchDirs', () => {
  it('collapses file entries to their dir, keeps dir entries as-is', () => {
    const config = {
      scripts: [{ in: 'src/js/main.ts', out: 'dist/main.js' }],
      styles: [{ in: 'src/scss/index.scss', out: 'dist/index.css' }],
      markup: { in: 'src/markup', out: 'dist' },
      copy: [{ in: 'src/static', out: 'dist' }]
    }
    expect(deriveWatchDirs(config).sort()).toEqual(
      ['src/js', 'src/scss', 'src/markup', 'src/static'].sort()
    )
  })

  it('includes reactor component and styles tokenPaths', () => {
    const config = {
      reactor: [{ component: 'src/js/App.jsx', in: 'src/js/hydrate.jsx', out: 'dist/h.js' }],
      styles: [{ in: 'src/scss/a.scss', out: 'x', options: { tokenPaths: ['src/tokens'] } }]
    }
    expect(deriveWatchDirs(config)).toContain('src/tokens')
    expect(deriveWatchDirs(config)).toContain('src/js')
    expect(deriveWatchDirs(config)).toContain('src/scss')
  })

  it('dedupes overlapping dirs', () => {
    const config = { scripts: [{ in: 'src/js/a.ts' }, { in: 'src/js/b.ts' }] }
    expect(deriveWatchDirs(config)).toEqual(['src/js'])
  })

  it('accepts single task objects and array `in`', () => {
    const config = { copy: { in: ['src/a', 'src/b'], out: 'dist' } }
    expect(deriveWatchDirs(config).sort()).toEqual(['src/a', 'src/b'].sort())
  })

  it('returns empty for no tasks and filters cwd `.`', () => {
    expect(deriveWatchDirs({})).toEqual([])
    expect(deriveWatchDirs({ copy: { in: '.', out: 'x' } })).toEqual([])
  })

  it('never watches node_modules from includePaths', () => {
    const config = { includePaths: ['node_modules'], markup: { in: 'src', out: 'dist' } }
    expect(deriveWatchDirs(config)).not.toContain('node_modules')
  })
})
