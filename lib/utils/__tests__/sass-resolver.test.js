...
describe('tryToFindFile', () => {
  ...
  // Known bug: path.format uses `base` over `name` when dir is present,
  // so the underscore prefix is silently ignored.
  it.failing('finds underscore partials at root level', () => {
  ...
  // Known bug: same path.format issue as above
  it.failing('finds files in subdirectories', () => {
    const result = tryToFindFile('src/subdir/item', ['sass', 'scss', 'css'])
    expect(result).toBe('src/subdir/_item.scss')
  })

  // Known bug: path.format ignores `name` when `base` is set.
  // When the input has a directory component AND an extension (e.g. "src/core/config.scss"),
  // path.parse sets `base`, and path.format uses `base` over `name`, so the underscore
  // prefix is silently dropped.
  // Known bug: same path.format issue
  it.failing('finds underscore partials in nested paths without extension', () => {
...
describe('sassPathResolver', () => {
  ...
  // Known bugs: underscore partial resolution fails for deep package paths.
  // The root cause is in tryToFindFile — path.format uses `base` over `name`,
  // so the underscore prefix added to `name` is silently ignored.
  it.failing('resolves my-pkg/core/config (underscore partial via package path)', () => {
  ...
  it.failing('resolves my-pkg/core/utils/helpers (underscore partial via package path)', () => {
