...
module.exports = class Styles {
  ...
  compileEntry(infilePath, outfilePath, options = {}) {
    ...
    try {
      ...
      console.log(`${pstyle.magentaBright + pstyle.bold}[style]${pstyle.reset} ${pstyle.redBright}[error]${pstyle.reset} ${pstyle.dim}Failed compiling:${pstyle.reset} ${pstyle.italic + pstyle.underline}${outfilePath}${pstyle.reset}`)
      console.log(err)
      return
    }
    ...
    if (options.minify) {
      postcss([autoprefixer, cssnano]).process(compiledSass.css, {
        ...
        console.log(`${pstyle.magentaBright + pstyle.bold}[style]${pstyle.reset} ${pstyle.redBright}[error]${pstyle.reset} ${pstyle.dim}Failed compiling:${pstyle.reset} ${pstyle.italic + pstyle.underline}${minPath}${pstyle.reset}`)
        console.log(err)
      })
