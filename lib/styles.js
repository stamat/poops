    ...
    for (const styleEntry of this.config.styles) {
      ...
      for (const entry of configured) {
        if (hasMagic(entry, { magicalBraces: true })) {
          ...
          // A glob-matched `index.scss` is named after its directory, otherwise
          // every component collides on `index.css`. Literal entries keep their
          // basename — see `isIndexEntry`.
          for (const match of matches) if (isIndexEntry(match)) indexEntries.add(match)
      ...
      for (const entryPoint of entryPoints) {
        ...
        const out = indexEntries.has(entryPoint) && !pathForFile(styleEntry.out)
          ? path.join(styleEntry.out, `${entryName(entryPoint)}.css`)
          : styleEntry.out
