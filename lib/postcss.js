...
export default class PostCSS {
  ...
  async compileEntry(infilePath, outfilePath, options = {}) {
    ...
    // PostCSS composes the input's map whenever the CSS carries a
    // sourceMappingURL. Leaving it unwritten leaves the previous stage's map
    // beside output whose lines this pass just moved — silently wrong.
    // The leading `;` shifts every mapping past the banner line, as styles.js does.
    if (result.map) {
      const map = result.map.toJSON()
      if (this.banner) map.mappings = ';' + map.mappings
      fs.writeFileSync(`${outfilePath}.map`, JSON.stringify(map))
      log({ tag: 'postcss', text: 'Compiled:', link: `${outfilePath}.map` })
    }
