...
module.exports = class Scripts {
  ...
  compileEntry(infilePath, outfilePath, options = {}) {
    ...
    build(opts).then(() => {
      ...
      for (const entry of infilePath) {
        ...
        if (options.minify) {
          ...
          Terser.minify(fs.readFileSync(newOutFilePath, 'utf-8'), { mangle: terserOpts.mangle }).then((result) => {
            if (result.error) {
              console.log(`${pstyle.yellowBright + pstyle.bold}[script]${pstyle.reset} ${pstyle.redBright}[error]${pstyle.reset} ${pstyle.dim}Failed compiling:${pstyle.reset} ${pstyle.italic + pstyle.underline}${minPath}${pstyle.reset}`)
      ...
      console.log(`${pstyle.yellowBright + pstyle.bold}[script]${pstyle.reset} ${pstyle.redBright}[error]${pstyle.reset} ${pstyle.dim}Failed compiling:${pstyle.reset} ${pstyle.italic + pstyle.underline}${outfilePath}${pstyle.reset}`)
      console.log(err)
    })
  }
}
