    ...
    for (const scriptEntry of this.config.scripts) {
      ...
      for (const entry of configured) {
        if (hasMagic(entry, { magicalBraces: true })) {
          ...
          // A glob-matched `index.js` is named after its directory, otherwise
          // every component collides on `index.js` or nests under esbuild's
          // outbase. Literal entries keep their basename — see `isIndexEntry`.
          entryPoints.push(...matches.map(match => (
            isIndexEntry(match) ? { in: match, out: entryName(match) } : match
          )))
