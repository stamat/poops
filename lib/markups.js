...
const { pathExists, pathIsDirectory, readDataFile, deleteDirectoryContents } = helpers
...
module.exports = class Markups {
  ...
  getCollectionData(collectionConfig) {
    ...
    for (let collection of collectionConfig) {
      ...
      deleteDirectoryContents(path.join(process.cwd(), this.config.markup.out, collection.name)) // Clear collection directory
      ...
      markupFiles.forEach((file) => {
        ...
        if (relativePathParts.length > 1 &&
          ...
          if (frontMatter.paginate && collectionData[collectionName].items.length > frontMatter.paginate) {
            ...
            collection.numberOfPages = collection.pages.length

            for (let i = 0; i < collection.numberOfPages; i++) {
              ...
              collection.nextPageUrl = collection.pageNumber === collection.numberOfPages ? null : `${collectionName}/${collection.pageNumber + 1}`
