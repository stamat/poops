...
export default class Markups {
  ...
  buildNavGlobal(markupIn, collectionData) {
    ...
    for (const file of files) {
      ...
      const outPath = this.mapOutputPath(path.join(process.cwd(), this.markupOut, relativePath))
      entries.push({
        ...frontMatter,
  ...
  async compileDirectory(markupIn, collectionData, pageEntries) {
    ...
    for (const file of markupFiles) {
      ...
      const markupOut = this.mapOutputPath(path.join(process.cwd(), this.markupOut, relativePath))
