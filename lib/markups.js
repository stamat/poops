...
const HTML_OUTPUT_EXTENSIONS = new Set(['.html', '.md', '.njk'])
...
export default class Markups {
  ...
  async compileDirectory(markupIn, collectionData, pageEntries) {
    ...
    for (const file of markupFiles) {
      ...
      const shouldIndex = pageEntries && HTML_OUTPUT_EXTENSIONS.has(path.extname(file))
      const fileCollection = relativePathParts.length > 1 && collectionData[relativePathParts[0]]
        ? relativePathParts[0]
        : null

      const compilePromise = this.compileEntry(file, fileContext).then(({ result, frontMatter }) => {
