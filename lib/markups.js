...
export default class Markups {
  ...
  compilePage(file, markupIn, collectionData, pageEntries) {
    ...
    const fileContext = {
      ...
      // source path relative to cwd/repo-root (posix), so templates can build
      // "edit on GitHub" links as package.homepage + /edit/<branch>/ + inputPath.
      // .html output can't be reversed to .md reliably; cwd is the repo root
      _page: { inputPath: toPosix(path.relative(process.cwd(), path.resolve(file))) }
    }
