...
const PrintStyle = require('./utils/print-pstyle.js')
    ...
    if (pathIsDirectory(markupIn)) {
      ...
      markupFiles.forEach((file) => {
        ...
        this.compileEntry(file, this.pkg).then((result) => {
          ...
          console.log(`${pstyle.cyanBright + pstyle.bold}[markup]${pstyle.reset} ${pstyle.dim}Compiled:${pstyle.reset} ${pstyle.italic + pstyle.underline}${path.relative(process.cwd(), markupOut)}${pstyle.reset}`)
        })
      })
    } else {
