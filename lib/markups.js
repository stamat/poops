import { pathExists, pathIsDirectory, readDataFile, deleteDirectory, mkDir, parseFrontMatter, clearFrontMatterCache, discoverImageVariants } from './utils/helpers.js'
...
export default class Markups {
  constructor(config) {
    ...
    this.nunjucksEnv.addFilter('srcset', (imagePath) => {
      const outputDir = path.join(process.cwd(), this.config.markup.out)
    ...
    this.nunjucksEnv.addExtension('ImageExtension', new ImageExtension(() => {
      return path.join(process.cwd(), this.config.markup.out)
    }))
    ...
    if (pathIsDirectory(markupIn)) {
      ...
      return new Promise((resolve, reject) => {
        Promise.all(compilePromises).then(() => {
          log({ tag: 'markup', text: `Compiled: ${markupFiles.length} file${markupFiles.length > 1 ? 's' : ''} into`, link: this.config.markup.out })
      ...
      return this.compileEntry(markupIn, collectionData).then((result) => {
        markupOut = this.replaceOutExtensions(markupOut)
        fs.writeFileSync(markupOut, result)
        log({ tag: 'markup', text: 'Compiled:', link: path.relative(process.cwd(), path.join(process.cwd(), this.config.markup.out, path.basename(markupIn))) })
        clearFrontMatterCache()
      }).catch((err) => {
        log({ tag: 'markup', error: true, text: 'Failed compiling:', link: path.relative(process.cwd(), path.join(process.cwd(), this.config.markup.out, path.basename(markupIn))) })
        console.error(err)
        clearFrontMatterCache()
      })
    }
