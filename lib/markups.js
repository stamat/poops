...
const { pathExists, pathIsDirectory, readDataFile, deleteDirectory, mkDir } = helpers
  ...
  compile() {
    ...
    if (pathIsDirectory(markupIn)) {
      ...
      markupFiles.forEach((file) => {
        ...
        if (relativePathParts.length > 1 &&
          ...
          if (frontMatter.paginate) {
            ...
            for (let i = 0; i < collection.totalPages; i++) {
              ...
              mkDir(markupOutDir)
              const relativeOutDir = path.relative(process.cwd(), collection.pageUrl, 'index.html')

              collectionData.relativePathPrefix = this.getRelativePathPrefix(relativeOutDir)
        ...
        mkDir(markupOutDir)
        const relativeOutDir = path.relative(process.cwd(), relativePath)
      ...
      mkDir(markupOutDir)
      const relativeOutDir = path.relative(process.cwd())

      const additionalContext = {}
      additionalContext.relativePathPrefix = this.getRelativePathPrefix(relativeOutDir)
