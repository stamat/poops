import { pathExists, pathIsDirectory, readDataFile, mkDir, buildTime, toPosix, fillPackageTokens } from './utils/helpers.js'
import { replaceOutExtensions, getRelativePathPrefix, getPageUrlRelativeToOutput, parseFrontMatter, wordcount, excerpt, hasTemplateTags, applyHeadingSlugs } from './markup/helpers.js'
import { renderMarkdownCached } from './markup/renderer.js'
...
import { openUpdateIndex, saveUpdateIndex } from './markup/update-index.js'
...
export default class Markups {
  constructor(config) {
    ...
    // Filled here rather than at render: `site` reaches pages as a global, the
    // search index, the feed and the sitemap, and all of them want the same
    // string. The tokens are the banner's, so `{{ version }}` means the same
    // thing in both halves of the config.
    this.siteData = fillPackageTokens(this.option('site') || {}, this.config.pkg)
    ...
    this.lastUpdatedConfig = this.option('lastUpdated')
  ...
  async compileEntry(templateName, additionalContext) {
    ...
    try {
      ...
      context.page.excerpt = await this.pageExcerpt(templateName, frontMatterResult.content, context)
    } catch (err) {
    ...
    const result = applyHeadingSlugs(await this.engine.render(templateName, context))
    return { result, frontMatter }
  }

  // page.excerpt is read in <head> — og, JSON-LD, meta description — while the
  // body it describes renders later, so it comes from the page's own markdown or
  // HTML rather than from the finished page.
  //
  // A first paragraph made of template tags (`{{ site.description }}`, an
  // included partial) has no text to take, so the body is rendered once with the
  // page's context to get it. Pages whose first paragraph is already prose never
  // pay for that render.
  //
  // A body that will not render warns; an engine with no renderString (the
  // contract does not require one) is silent, as it would warn on every page.
  // Either way the excerpt stays empty, dropping og/JSON-LD to site.description
  // rather than to a wrong string.
  async pageExcerpt(templateName, content, context) {
    const body = path.extname(templateName) === '.md' ? renderMarkdownCached(templateName, content) : content
    const text = excerpt(body)
    if (text || !hasTemplateTags(body)) return text
    if (typeof this.engine.renderString !== 'function') return ''

    try {
      return excerpt(await this.engine.renderString(body, context))
    } catch (err) {
      log({ tag: this.logTag, warn: true, text: 'Could not resolve the excerpt of:', link: path.relative(process.cwd(), templateName) })
      return ''
    }
  }
  ...
  async compile() {
    ...
    // Before any front matter is parsed — that is where pages take their
    // `updated` date from, collection items included
    openUpdateIndex(this.lastUpdatedConfig, pathIsDirectory(markupIn) ? markupIn : path.dirname(markupIn))
    ...
    // Every consumer of pageEntries has to be named here — an index feature left
    // out of this list silently generates nothing, since generateIndexFiles is
    // handed a null page set.
    const shouldIndex = this.searchIndexConfig || this.sitemapConfig || this.llmsConfig || this.navConfig || this.feedConfig
    ...
    // A full build saw every page, so it is the only one allowed to drop
    // entries for sources that are gone
    saveUpdateIndex({ prune: true })
...
function entrySignature(entry) {
  const { content, wordcount, excerpt, updated, ...rest } = entry
