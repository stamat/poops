import { pathExists, pathIsDirectory, readDataFile, mkDir, parseFrontMatter, clearFrontMatterCache, discoverImageVariants } from './utils/helpers.js'
...
export default class Markups {
  constructor(config) {
    ...
    this.nunjucksEnv.addFilter('date', (str, template) => {
      if (!template) template = this.timeDateFormat
    ...
    this.nunjucksEnv.addFilter('srcset', (imagePath) => {
      const outputDir = path.join(process.cwd(), this.markupOut)
